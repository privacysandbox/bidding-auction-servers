...
while getopts "s:" flag; do
 case $flag in
   s) # Handle the -s flag with an argument
   service=$OPTARG
   ;;
   \?)
...
echo "value of service flag passed in with -s: $service"

if [[ $service == "bidding" ]] || [[ $service == "auction" ]]; then
    echo "service was either 'bidding' or 'auction'"
fi
...
APPMESH_INGRESS_IGNORED_PORTS="443"
