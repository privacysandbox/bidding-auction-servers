...
#ifndef SERVICES_BUYER_REPORTING_UDF_FETCH_MANAGER_H_
...
class BuyerReportingUdfFetchManager : public FetcherInterface {
 public:
  explicit BuyerReportingUdfFetchManager(
      ...
      : config_(*config),
        ...
        executor_(*executor),
        buyer_code_wrapper_(std::move(buyer_code_wrapper)),
        dispatcher_(*dispatcher) {}

  ~BuyerReportingUdfFetchManager() { End(); }

  // Not copyable or movable.
  BuyerReportingUdfFetchManager(const BuyerReportingUdfFetchManager&) = delete;
  BuyerReportingUdfFetchManager& operator=(
      const BuyerReportingUdfFetchManager&) = delete;
 ...
 private:
  absl::Status PeriodicBuyerReportingFetchAndLoadSync();
  ...
  WrapSingleCodeBlobForDispatch buyer_code_wrapper_;
...
#endif  // SERVICES_BUYER_REPORTING_UDF_FETCH_MANAGER_H_
