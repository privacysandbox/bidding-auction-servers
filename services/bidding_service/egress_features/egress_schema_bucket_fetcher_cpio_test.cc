...
class CpioEgressSchemaBucketFetcherTest : public ::testing::Test {
 protected:
  void SetUp() override {
    CommonTestInit();
...
TEST_F(CpioEgressSchemaBucketFetcherTest, UpdatesCacheWithSuccessfulFetches) {
  ...
  std::unique_ptr<EgressSchemaBucketFetcher> fetcher =
      std::make_unique<EgressSchemaBucketFetcher>(
          ...
          cpio_client.get(), egress_schema_cache_.get());
