...
MultiCurlHttpFetcherAsyncNoQueue::MultiCurlHttpFetcherAsyncNoQueue(
...
void MultiCurlHttpFetcherAsyncNoQueue::ShutdownEventLoop(int fd,
                                                         short event_type,
                                                         void* arg) {
  auto* self = reinterpret_cast<MultiCurlHttpFetcherAsyncNoQueue*>(arg);
...
void MultiCurlHttpFetcherAsyncNoQueue::FetchUrls(
...
void MultiCurlHttpFetcherAsyncNoQueue::FetchUrlsWithMetadata(
    const std::vector<HTTPRequest>& requests, absl::Duration timeout,
  ...
  // Using a shared_ptr here allows us to avoid making
  // MultiCurlHttpFetcherAsyncNoQueue the owner of the FetchUrlsLifetime, which
  // would complicate cleanup on MultiCurlHttpFetcherAsyncNoQueue destruction
  // during a pending FetchUrls call.
  auto shared_lifetime = std::make_shared<
      MultiCurlHttpFetcherAsyncNoQueue::FetchUrlsWithMetadataLifetime>();
...
void MultiCurlHttpFetcherAsyncNoQueue::ExecuteCurlRequest(
...
void MultiCurlHttpFetcherAsyncNoQueue::FetchUrl(const HTTPRequest& request,
                                                int timeout_ms,
...
void MultiCurlHttpFetcherAsyncNoQueue::FetchUrlWithMetadata(
...
std::unique_ptr<CurlRequestData>
MultiCurlHttpFetcherAsyncNoQueue::CreateCurlRequest(
...
void MultiCurlHttpFetcherAsyncNoQueue::PutUrl(const HTTPRequest& http_request,
                                              int timeout_ms,
                                              OnDoneFetchUrl done_callback) {
...
std::pair<absl::Status, void*>
MultiCurlHttpFetcherAsyncNoQueue::GetResultFromMsg(CURLMsg* msg) {
  void* output;
  absl::Status status;
...
void MultiCurlHttpFetcherAsyncNoQueue::PerformCurlUpdate()
    ...
    executor_->Run([req_handle = msg->easy_handle, status = status,
      ...
      if (data_ptr == nullptr) {
        ABSL_LOG(ERROR) << "Curl Error: Pointer to Curl data lost with status: "
                        << status << ". Memory for this call has leaked.";
        return;
      }
...
void MultiCurlHttpFetcherAsyncNoQueue::Add(CURL* handle) {
  // Add request handle to set if required for cleanup.
  absl::MutexLock lock(&curl_handle_set_lock_);
  curl_handle_set_.emplace(handle);
}

void MultiCurlHttpFetcherAsyncNoQueue::Remove(CURL* handle) {
  absl::MutexLock lock(&curl_handle_set_lock_);
