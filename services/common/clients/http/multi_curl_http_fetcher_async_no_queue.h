...
#ifndef SERVICES_COMMON_CLIENTS_MULTI_CURL_HTTP_FETCHER_ASYNC_NO_QUEUE_H_
#define SERVICES_COMMON_CLIENTS_MULTI_CURL_HTTP_FETCHER_ASYNC_NO_QUEUE_H_
...
#include "services/common/clients/http/multi_curl_request_manager_locking.h"
...
// MultiCurlHttpFetcherAsyncNoQueue provides a thread-safe libcurl wrapper to
// perform asynchronous HTTP invocations with client caching(connection
// pooling), and TLS session sharing. It uses a single curl multi handle to
// perform all invoked HTTP actions. It runs a loop in the provided executor to
// provide computation to Libcurl for I/O, and schedules callbacks on the
// provided executor with the result from the HTTP invocation. Please note:
// MultiCurlHttpFetcherAsyncNoQueue makes the best effort to reject any calls
// after the class has started shutting down but does not guarantee thread
// safety if any method is invoked after destruction.
class MultiCurlHttpFetcherAsyncNoQueue final : public HttpFetcherAsync {
 public:
  ...
  explicit MultiCurlHttpFetcherAsyncNoQueue(
  ...
  // from any threads.
  ~MultiCurlHttpFetcherAsyncNoQueue() override
      ABSL_LOCKS_EXCLUDED(in_loop_mu_, curl_handle_set_lock_);

  // Not copyable or movable.
  MultiCurlHttpFetcherAsyncNoQueue(const MultiCurlHttpFetcherAsyncNoQueue&) =
      delete;
  MultiCurlHttpFetcherAsyncNoQueue& operator=(
      const MultiCurlHttpFetcherAsyncNoQueue&) = delete;
 ...
 private:
  ...
  MultiCurlRequestManagerLocking multi_curl_request_manager_;
...
#endif  // SERVICES_COMMON_CLIENTS_MULTI_CURL_HTTP_FETCHER_ASYNC_NO_QUEUE_H_
