...
void MultiCurlRequestManagerLocking::OnLibeventSocketActivity(int fd,
                                                              short kind,
                                                              void* data) {
  int action = ((kind & EV_READ) ? CURL_CSELECT_IN : 0) |
               ((kind & EV_WRITE) ? CURL_CSELECT_OUT : 0);

  auto* self = reinterpret_cast<MultiCurlRequestManagerLocking*>(data);
  {
...
void MultiCurlRequestManagerLocking::UpsertSocketInLibevent(
    curl_socket_t sock_fd, int activity, SocketInfo* socket_info) {
...
void MultiCurlRequestManagerLocking::AddSocketToLibevent(curl_socket_t sock,
                                                         int activity,
                                                         void* data) {
...
int MultiCurlRequestManagerLocking::OnLibcurlSocketUpdate(
    CURL* easy_handle, curl_socket_t sock_fd, int activity, void* data,
...
MultiCurlRequestManagerLocking::MultiCurlRequestManagerLocking(
...
// static
void MultiCurlRequestManagerLocking::MultiTimerCallback(int fd, short what,
                                                        void* arg) {
  auto* self = reinterpret_cast<MultiCurlRequestManagerLocking*>(arg);
  {
    absl::MutexLock l(&self->request_manager_mu_);
...
void MultiCurlRequestManagerLocking::Configure(
...
CURLMcode MultiCurlRequestManagerLocking::Add(CURL* curl_handle)
...
CURLMsg* MultiCurlRequestManagerLocking::GetUpdate(int* msgs_left)
    ABSL_LOCKS_EXCLUDED(request_manager_mu_) {
  absl::MutexLock l(&request_manager_mu_);
...
CURLMcode MultiCurlRequestManagerLocking::Remove(CURL* curl_handle)
    ABSL_LOCKS_EXCLUDED(request_manager_mu_) {
  absl::MutexLock l(&request_manager_mu_);
