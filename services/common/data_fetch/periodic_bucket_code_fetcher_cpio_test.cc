...
class CpioPeriodicBucketCodeFetcherTest : public ::testing::Test {
 protected:
  void SetUp() override { CommonTestInit(); }
};

TEST_F(CpioPeriodicBucketCodeFetcherTest, LoadsWrappedResultIntoCodeLoader) {
  ...
  CpioBlobStorageClient cpio_client =
      CpioBlobStorageClient(std::move(blob_storage_client));

  PeriodicBucketCodeFetcher bucket_fetcher(kSampleBucketName, kFetchPeriod,
                                           &dispatcher, executor.get(),
                                           std::move(wrapper), &cpio_client);
  auto status = bucket_fetcher.Start();
  ASSERT_TRUE(status.ok()) << status;
  bucket_fetcher.End();
}

TEST_F(CpioPeriodicBucketCodeFetcherTest, LoadsAllBlobsInBucket) {
  MockUdfCodeLoaderInterface dispatcher;
  auto executor = std::make_unique<MockExecutor>();
  auto blob_storage_client = std::make_unique<MockBlobStorageClient>();
...
TEST_F(CpioPeriodicBucketCodeFetcherTest, ReturnsSuccessIfAtLeastOneBlobLoads) {
  MockUdfCodeLoaderInterface dispatcher;
  auto executor = std::make_unique<MockExecutor>();
  auto blob_storage_client = std::make_unique<MockBlobStorageClient>();
...
TEST_F(CpioPeriodicBucketCodeFetcherTest,
